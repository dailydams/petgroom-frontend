            ...
            fileInput.addEventListener('change', async (e) => {
                ...
                if (file) {
                    try {
                        ...
                        console.log('엑셀 일괄등록 시작:', file.name);
                        
                        // API 호출 - 개선된 importCustomers 함수 사용
                        const result = await API.importCustomers(formData);
                        console.log('일괄등록 API 응답:', result);
                        
                        if (result && result.success) {
                            // 고객 데이터를 메모리에 업데이트
                            if (result.customers && result.customers.length > 0) {
                                console.log('일괄등록 성공:', result.customers.length, '명의 고객 정보가 등록됨');
                                
                                // 로컬 변수에 복사본 생성
                                const newCustomers = [...customers];
                                
                                result.customers.forEach(newCustomer => {
                                    // 중복 고객 확인 (전화번호로 비교)
                                    const existingIndex = newCustomers.findIndex(c => c.phone === newCustomer.phone);
                                ...
                                try {
                                    ...
                                    console.log('고객 데이터가 로컬 스토리지에 저장되었습니다.');
                                } catch (storageError) {
                            ...
                            ToastNotification.show(`${result.customers ? result.customers.length : 0}명의 고객 정보가 성공적으로 등록되었습니다.`, 'success');
                        } else {
...
function renderAppointmentsForStaff(container, staffId) {
    ...
    dailyAppointments.forEach(app => {
        ...
        appointmentItem.addEventListener('click', (e) => {
            ...
            // 예약 상태가 'reserved' 또는 'scheduled'인 경우 편집 모달 열기
            if (app.status === 'reserved' || app.status === 'scheduled') {
                openAppointmentModal(null, null, null, app);
            } else {
                // 그 외 상태(완료 등)인 경우 매출 등록 모달 열기
                openSaleModal(app);
            }
        });
...
async function loadMonthView() {
    try {
        console.log('월간 뷰 로드 시작');
        const monthBody = document.getElementById('month-body');
        if (!monthBody) {
            console.error('month-body 요소를 찾을 수 없습니다.');
            return;
        }
        ...
        weekdayHeader.className = 'month-week weekday-header';
        const weekdays = ['일', '월', '화', '수', '목', '금', '토'];
        ...
        monthBody.appendChild(weekdayHeader);
        ...
        console.log('월간 뷰: 현재 월', `${currentDate.getFullYear()}년 ${currentDate.getMonth() + 1}월`);
        console.log('월간 뷰: 첫날/마지막날', firstDay, lastDay);
        ...
        console.log('월간 뷰: 캘린더 범위', firstWeekStart, lastWeekEnd);
        ...
        try {
            ...
            console.log('월간 뷰: 예약 데이터', monthAppointments.length, '건 로드됨');
        } catch (error) {
...
// 예약 모달 열기
async function openAppointmentModal(time = null, staffId = null, date = null, appointmentData = null) {
    try {
        ...
        if (modalTitle) {
            modalTitle.textContent = appointmentData ? '예약 편집' : '신규 예약';
        }
        
        // 기존 예약 데이터가 있는 경우 폼에 로드
        if (appointmentData) {
            // 예약 ID 저장
            form.dataset.appointmentId = appointmentData.id;
            
            // 날짜 및 시간 설정
            document.getElementById('appointment-date').value = appointmentData.date;
            ...
            // 고객 정보 설정
            const guardian = appointmentData.guardian || {};
            document.getElementById('guardian-name').value = guardian.name || '';
            document.getElementById('guardian-phone').value = guardian.phone || '';
            document.getElementById('guardian-memo').value = guardian.memo || '';
            document.getElementById('alimtalk-consent').checked = guardian.alimtalkConsent !== false;
            
            // 서비스 설정
            document.getElementById('selected-service').value = appointmentData.service || '';
            
            // 서비스 버튼 활성화
            const serviceButtons = document.querySelectorAll('.service-btn');
            serviceButtons.forEach(btn => {
                if (btn.dataset.service === appointmentData.service) {
                    btn.classList.add('active');
                } else {
            ...
            // 알림톡 옵션
            let alimtalkOption = 'none';
            if (typeof appointmentData.alimtalk === 'object') {
                alimtalkOption = appointmentData.alimtalk.option || 'none';
            } else {
                alimtalkOption = appointmentData.alimtalk || 'none';
            }
            
            const alimtalkRadio = document.querySelector(`input[name="alimtalk-option"][value="${alimtalkOption}"]`);
            if (alimtalkRadio) alimtalkRadio.checked = true;
            
            // 동의서
            const agreements = appointmentData.agreements || {};
            document.getElementById('agreement-default').checked = !!agreements.default;
            document.getElementById('agreement-senior').checked = !!agreements.senior;
            
            // 메모
            document.getElementById('appointment-memo').value = appointmentData.memo || '';
            
            // 담당자 설정 (아래 staffId 로직에서 처리됨)
            staffId = appointmentData.staff ? appointmentData.staff.id : null;
            
            // 반려동물 정보 로드
            const petsContainer = document.getElementById('pet-containers');
            petsContainer.innerHTML = '';
            
            if (appointmentData.pets && appointmentData.pets.length > 0) {
                appointmentData.pets.forEach((pet, index) => {
                    ...
                    petContainer.innerHTML = `
                        <div class="form-grid">
                            <div class="form-group">
                                <label for="pet-name-${petIndex}">반려동물명</label>
                                <input type="text" id="pet-name-${petIndex}" name="pet_name_${petIndex}" class="form-control pet-name" value="${pet.name || ''}" required>
                            </div>
                            <div class="form-group">
                                <label for="pet-breed-${petIndex}">품종</label>
                                <input type="text" id="pet-breed-${petIndex}" name="pet_breed_${petIndex}" class="form-control pet-breed" value="${pet.breed || ''}" required>
                            </div>
                        </div>
                        <div class="form-grid">
                            <div class="form-group">
                                <label for="pet-weight-${petIndex}">몸무게(kg)</label>
                                <input type="number" id="pet-weight-${petIndex}" name="pet_weight_${petIndex}" class="form-control pet-weight" step="0.1" value="${pet.weight || ''}" required>
                            </div>
                            <div class="form-group">
                                <label for="pet-age-${petIndex}">나이</label>
                                <input type="number" id="pet-age-${petIndex}" name="pet_age_${petIndex}" class="form-control pet-age" value="${pet.age || ''}">
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="pet-memo-${petIndex}">반려동물 메모</label>
                            <textarea id="pet-memo-${petIndex}" name="pet_memo_${petIndex}" class="form-control pet-memo">${pet.memo || ''}</textarea>
                        </div>
                    `;
        ...
        // 반려동물 입력 필드에 검색 자동완성 이벤트 추가
        const petNameInputs = document.querySelectorAll('.pet-name');
        petNameInputs.forEach(petNameInput => {
            const searchResultsContainer = document.createElement('div');
            searchResultsContainer.className = 'search-results';
            searchResultsContainer.style.display = 'none';
            petNameInput.parentNode.appendChild(searchResultsContainer);
                ...
                try {
                    ...
                    if (customers && customers.length > 0) {
                        ...
                        matchingCustomers.forEach(customer => {
                            ...
                            matchingPets.forEach(pet => {
                                ...
                                resultItem.innerHTML = `
                                    <strong>${pet.name}</strong> - ${pet.breed || '품종 미상'}, ${pet.weight || '?'}kg
                                    <br><small>${customer.name} (${customer.phone})</small>
                                `;
                                
                                resultItem.addEventListener('click', () => {
                                    // 선택한 반려동물 및 고객 정보로 폼 채우기
                                    petNameInput.value = pet.name || '';
                                    
                                    // 같은 행의 다른 필드들 채우기
                                    const petContainer = petNameInput.closest('.pet-container');
                                    const breedInput = petContainer.querySelector('.pet-breed');
                                    const weightInput = petContainer.querySelector('.pet-weight');
                                    const ageInput = petContainer.querySelector('.pet-age');
                                    const memoInput = petContainer.querySelector('.pet-memo');
                                    
                                    if (breedInput) breedInput.value = pet.breed || '';
                                    if (weightInput) weightInput.value = pet.weight || '';
                                    if (ageInput) ageInput.value = pet.age || '';
                                    if (memoInput) memoInput.value = pet.memo || '';
                                    
                                    // 보호자 정보 채우기
                                    document.getElementById('guardian-name').value = customer.name || '';
                                    document.getElementById('guardian-phone').value = customer.phone || '';
                        ...
                        try {
                            ...
                            console.error('API 고객 검색 실패:', error);
                        }
                    }
                } catch (error) {
                    console.error('반려동물 검색 중 오류:', error);
                    searchResultsContainer.style.display = 'none';
                }
            }, 300));
        });
        
        // 보호자 이름 입력 필드에 검색 자동완성 이벤트 추가
                ...
                if (customers && customers.length > 0) {
                    ...
                    matchingCustomers.forEach(customer => {
                        ...
                        resultItem.innerHTML = `
                            ...
                            <br><small>${customer.pets ? customer.pets.map(p => p.name).join(', ') : '반려동물 없음'}</small>
                        `;
...
function renderCustomersTable() {
    try {
        ...
        // 메모리에서 고객 데이터 가져오기 (전역 변수 사용)
        // 이전: const customers = JSON.parse(localStorage.getItem('customers') || '[]');
        // 메모리의 고객 데이터 사용
        const customersData = customers || [];
        
        console.log('고객 테이블 렌더링: 총', customersData.length, '명의 고객 정보');
...
// 보호자 이름 입력 시 검색 결과 처리
function handleCustomerSearchResult(customer, guardianNameInput, guardianPhoneInput, searchResultsContainer) {
    // 고객 정보 자동 입력
    guardianNameInput.value = customer.name || '';
    guardianPhoneInput.value = customer.phone || '';
    
    // 알림톡 동의 체크
    const alimtalkConsent = document.getElementById('alimtalk-consent');
    if (alimtalkConsent && customer.alimtalkConsent) {
        alimtalkConsent.checked = customer.alimtalkConsent;
    }
    
    // 보호자 메모 입력
    const guardianMemo = document.getElementById('guardian-memo');
    if (guardianMemo && customer.memo) {
        guardianMemo.value = customer.memo;
    }
    
    // 반려동물 정보가 있으면 기존 컨테이너 초기화
    if (customer.pets && customer.pets.length > 0) {
        ...
        if (petContainers) {
            ...
            customer.pets.forEach((pet, index) => {
                addPetForm(); // 새 컨테이너 추가
                
                // 추가된 마지막 컨테이너 가져오기
                const allContainers = document.querySelectorAll('#pet-containers .pet-container');
                const petContainer = allContainers[allContainers.length - 1];
                
                if (petContainer) {
                    // 반려동물 정보 입력
                    const nameInput = petContainer.querySelector('.pet-name');
                    const breedInput = petContainer.querySelector('.pet-breed');
                    const weightInput = petContainer.querySelector('.pet-weight');
                    const ageInput = petContainer.querySelector('.pet-age');
                    const memoInput = petContainer.querySelector('.pet-memo');
    ...
    console.log('고객 정보 자동완성 처리 완료:', customer.name);
}
...
function openCustomerModal(customerId = null, mode = 'add') {
    try {
        ...
        if (!modal || !form) {
            console.error('고객 모달 또는 폼을 찾을 수 없습니다.');
            return;
        }
        ...
        if (modalTitle) {
            if (mode === 'view') {
                modalTitle.textContent = '고객 상세 정보';
            } else if (mode === 'edit') {
        ...
        const addPetBtn = document.getElementById('customer-add-pet-btn');
        
        if (saveBtn) saveBtn.style.display = isReadOnly ? 'none' : 'block';
        if (addPetBtn) addPetBtn.style.display = isReadOnly ? 'none' : 'block';
        if (cancelBtn) cancelBtn.textContent = isReadOnly ? '닫기' : '취소';
        
        // 기존 고객 정보 불러오기
        if (customerId) {
            console.log('고객 정보 조회 시작:', customerId);
            ...
            if (customer) {
                ...
                if (alimtalkConsent) {
                    alimtalkConsent.checked = customer.alimtalkConsent || false;
                }
                ...
                if (petContainers) {
                    ...
                    if (customer.pets && customer.pets.length > 0) {
                        ...
                        customer.pets.forEach((pet, index) => {
                            ...
                            // 추가된 반려동물 컨테이너에 정보 설정
                            const containers = petContainers.querySelectorAll('.pet-container');
                            const container = containers[containers.length - 1]; // 마지막 추가된 컨테이너
                            
                            if (container) {
                                // 필드 ID 계산 (1부터 시작)
                                const petIndex = containers.length;
                                
                                // 각 필드 설정
                                const nameInput = container.querySelector(`#customer-pet-name-${petIndex}`);
                                const breedInput = container.querySelector(`#customer-pet-breed-${petIndex}`);
                                const weightInput = container.querySelector(`#customer-pet-weight-${petIndex}`);
                                const ageInput = container.querySelector(`#customer-pet-age-${petIndex}`);
                                const memoInput = container.querySelector(`#customer-pet-memo-${petIndex}`);
                        ...
                        // 반려동물 정보가 없는 경우 기본 폼 하나 추가
                        addCustomerPetForm();
                        
                        // 읽기 전용 모드인 경우 입력 필드 비활성화
                        if (isReadOnly) {
                            petContainers.querySelectorAll('input, textarea').forEach(input => {
                                input.readOnly = true;
                            });
                            
                            // 삭제 버튼 숨기기
                            petContainers.querySelectorAll('.remove-pet-btn').forEach(btn => {
                ...
                console.error('고객 정보를 찾을 수 없습니다:', customerId);
